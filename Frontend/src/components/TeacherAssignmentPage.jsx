...
const TeacherAssignmentPage = ({ onBack, onNext }) => {
  ...
  useEffect(() => {
    const fetchTeachersAndSubjects = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/teachers-subjects');
        if (!response.ok) {
        ...
        setTeachers(data.teachers.map(teacher => ({
          ...teacher,
          subject_preferences: teacher.subject_preferences || [] // Ensure subject_preferences is an array
        })));
        setSubjects(data.subjects.map(subject => ({
          ...subject,
          weekly_load: subject.weekly_load || '0,0' // Ensure weekly_load is a string, default to '0,0'
        })));

        // Initialize workload summary and assignments
        const initialWorkload = {};
        data.teachers.forEach(teacher => {
          initialWorkload[teacher.mis_id] = {
            assigned: 0,
        ...
        data.subjects.forEach(subject => {
          if (subject.assignedTeachers && subject.assignedTeachers.length > 0) {
            const assignedTeacherId = subject.assignedTeachers[0].mis_id; // Assuming mis_id is used for assignment
            initialAssignments[subject._id] = assignedTeacherId;

            // Update workload for initially assigned teachers
            if (initialWorkload[assignedTeacherId]) {
              initialWorkload[assignedTeacherId].assigned += subject.total_hours;
              initialWorkload[assignedTeacherId].remaining -= subject.total_hours;
              initialWorkload[assignedTeacherId].subjects.push(subject.code); // Add subject code to the array
            }
          }
        });

        setWorkloadSummary(initialWorkload);
